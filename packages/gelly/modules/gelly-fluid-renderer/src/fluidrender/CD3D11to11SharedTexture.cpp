...
bool CD3D11to11SharedTexture::Create() {
	...
	// Some shared surface implementations only support a subset of formats.
	auto format = GetDXGIFormat(autoGeneratedDesc.format);
...
void CD3D11to11SharedTexture::CopyToTexture(
	GellyInterfaceRef<IManagedTexture> texture
) {
	if (!context) {
		throw std::logic_error(
			"CD3D11to11SharedTexture::CopyToTexture() encountered a null "
			"context"
		);
	}
