...
void StandardPipeline::Composite() {
	...
	device->SetTexture(0, textures->gmodTextures[frameIndex].depth.Get());
	device->SetTexture(1, textures->gmodTextures[frameIndex].normal.Get());
	device->SetTexture(2, backBuffer.Get());
	device->SetTexture(3, textures->gmodTextures[frameIndex].thickness.Get());
	device->SetTexture(4, GetCubemap());
	device->SetTexture(5, textures->gmodTextures[frameIndex].albedo.Get());
...
void StandardPipeline::CompositeFoam(bool writeDepth) {
	...
	device->SetTexture(0, textures->gmodTextures[frameIndex].thickness.Get());
	device->SetTexture(1, textures->gmodTextures[frameIndex].depth.Get());
