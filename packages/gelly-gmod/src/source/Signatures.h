...
constexpr const char *const CMaterialSystem_AllowThreading =
	...
	"0f "
	"b6 da 48 8b f9 ff ?? ?? ?? ?? ?? 41 b8 02 00 00 00 48 ?? ?? ?? ?? ?? "
	"?? "
	"48 8b c8 4c 8b 08 41 ff ?? ?? ?? ?? ?? 99 30 00 02 33 c9 83 f8 02 0f "
	"4c "
	"d9 ff ?? ?? ?? ?? ?? 80 78 05 02 ?? ?? 8b 87 58 2f 00 02 88 9f 99 30 "
	"00 "
	"02 84 db";
...
constexpr const char *const CShaderAPIDX8_SetAmbientLightCube =
	...
	"f1 48 8b cf 41 b8 60 00 00 00 48 8b ?? ?? ?? ?? 02 00";
