 ...
 * @note This function uses the special window class name "Valve001" to find the
 * Garry's Mod window.
 * @note If another window has the same window class name, there is no guarantee
 * that this function will return the correct window handle.
 * @return
 */
static HWND GetGModWindow() { return FindWindowA("Valve001", nullptr); }

/**
 * Gets the IDirect3DDevice9 vtable. Can fail, so check the return value. If
 * true, the vtable is written to destinationVTable.
...
static bool GetD3D9VTable(D3D9VTable destinationVTable) {
	...
	if (device == nullptr) {
		...
		sprintf_s(
			error,
			"Failed to create the D3D9 device. Error code: 0x%lx\nHWND: %d",
			hr,
			params.hDeviceWindow
		);
	...
	// We're going to release this device, so we'll copy the VTable instead of
	// just writing it.
	memcpy(
		destinationVTable,
