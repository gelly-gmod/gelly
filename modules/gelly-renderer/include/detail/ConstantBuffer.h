...
public:
    explicit ConstantBuffer(ID3D11Device* device) {
        static_assert((sizeof(Structure) % 16) == 0, "Constant Buffer size must be 16-byte aligned");
        ...
        DX("Failed to make constant buffer", device->CreateBuffer(&desc, nullptr, buffer.GetAddressOf()));
    }
    ...
    void Set(ID3D11DeviceContext* context, Structure* newData) {
        ...
        DX("Failed to map constant buffer", context->Map(buffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource));
