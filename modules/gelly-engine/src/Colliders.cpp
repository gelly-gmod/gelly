...
/**
 * @brief This is a monotonic increasing counter that is used to assign unique
 * IDs to entities.
 */
...
void Colliders::AddTriangleMesh(
	...
	NvFlexBuffer *indicesBuffer = NvFlexAllocBuffer(
		library, info.indexCount, sizeof(int), eNvFlexBufferHost
	);

	Vec4 *vertices =
		static_cast<Vec4 *>(NvFlexMap(verticesBuffer, eNvFlexMapWait));
	int *indices = static_cast<int *>(NvFlexMap(indicesBuffer, eNvFlexMapWait));

	for (int i = 0; i < info.vertexCount; i++) {
		vertices[i] = Vec4{
			info.vertices[i].x, info.vertices[i].y, info.vertices[i].z, 0.f};
	}
	...
	NvFlexUpdateTriangleMesh(
		...
		info.vertexCount,
		info.vertexCount / 3,
		reinterpret_cast<const float *>(&info.lower),
		reinterpret_cast<const float *>(&info.upper)
	);
	...
	for (int entityIndex = 0; entityIndex < entities.size(); entityIndex++) {
		...
		switch (mesh.geometryType) {
			case eNvFlexShapeTriangleMesh:
				...
				geometries[entityIndex].triMesh.scale[0] =
					mesh.properties.triangleMesh.scale.x;
				geometries[entityIndex].triMesh.scale[1] =
					mesh.properties.triangleMesh.scale.y;
				geometries[entityIndex].triMesh.scale[2] =
					mesh.properties.triangleMesh.scale.z;

				flags[entityIndex] =
					NvFlexMakeShapeFlags(eNvFlexShapeTriangleMesh, true);
				break;
			case eNvFlexShapeSphere:
				geometries[entityIndex].sphere.radius =
					mesh.properties.sphere.radius;

				flags[entityIndex] =
					NvFlexMakeShapeFlags(eNvFlexShapeSphere, true);
				break;
			case eNvFlexShapeCapsule:
				geometries[entityIndex].capsule.radius =
					mesh.properties.capsule.radius;
				geometries[entityIndex].capsule.halfHeight =
					mesh.properties.capsule.halfHeight;

				flags[entityIndex] =
					NvFlexMakeShapeFlags(eNvFlexShapeCapsule, true);
				break;
			default:
		...
		positions[entityIndex] =
			Vec4{entity.position.x, entity.position.y, entity.position.z, 0.5f};
